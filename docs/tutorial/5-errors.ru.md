...
Обе дороги блока заканчиваются одинаково. Дорога, которая просто кончилась
(`"next": null`), возвращает управление узлу `try`, и граф идёт дальше с его
собственного `next` — неважно, тело это было или обработчик.

Поэтому стадия с правилами может и назвать следующий узел, и обойтись без
`next`: в обоих случаях граф продолжится с `gather`:
...
Исключение — узел `terminal` внутри блока: он завершает весь запуск, и ничего
после блока не исполняется.
