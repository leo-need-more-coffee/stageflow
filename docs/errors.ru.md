...
- Дорога, которая просто кончилась (`"next": null`), возвращает управление
  блоку, и тот продолжает со своего `next`. Для тела и для обработчика это
  работает одинаково: обработчик — часть блока, а не выход из него.
- Узел `terminal` внутри блока завершает весь запуск, и ничего после блока не
  исполняется.
