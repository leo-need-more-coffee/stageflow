...
Используется везде, где в пайплайне встречается
выражение: ``condition`` у condition-ноды, ``when`` у switch-ноды и любое
значение с суффиксом ``.$`` в ``arguments``/``outputs``/``variables``.
Переменные фрейма живут в активации под namespace ``vars``: префикс нужен,
чтобы переменная с именем функции CEL (``size``, ``type``, ``has``) ни с чем
не схлопывалась.
